...
# print(next(os.walk(path))) # to watch the content os walk function

# looping throug this folder
for dirpath, _, files in os.walk(path):

    # get the name of directory where this file is in
    challenge = os.path.basename(dirpath)

    # evaluates if this directoy start with the patern
    if re.match(CHALLENGE_PATTERN, challenge):
        
        # change the value of current to the name of the dirctory 
    ...
    for file in files:
        if file.lower() not in EXCLUDED:
            language = os.path.basename(dirpath).lower() # the same is -> challenge.lower()
            languages[language] = languages.get(language, 0) + 1 # adds 1 to the value of language key

            # TODO: continue commenting this code
